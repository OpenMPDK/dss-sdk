...
// (number of JPs) needed.
        ...
        digitshincr       = cJU_UNSIGNED_ONE << (cJU_BITSPERBYTE * (levelsub - 1));
        retval            = TRUE;
...
// completely stored:
            ...
            {                                   // complete success.
                ++numJPs;
#if !defined JU_KM || defined JU_WIN
                DBGCODE(assert(pop1subprev == pop1sub));
#endif
                assert(pop1 >= pop1sub);
...
#if !defined JU_KM || defined JU_WIN
            DBGCODE(assert(pop1subprev > pop1sub));      // check j__udyInsArray().
#endif
