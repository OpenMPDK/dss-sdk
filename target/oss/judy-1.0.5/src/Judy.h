...
typedef void void_t;
typedef void_t *handle;
typedef void_t *pvoid_t;
typedef void_t *Pvoid_t;
typedef void_t **PPvoid_t;
typedef const void_t *Pcvoid_t;
typedef void_t *HANDLE;

#define _pprocid_t uint64_t

#if defined(__X86_64__)
//#pragma message ("Compiling for Linux x86_64")
#define JU_64BIT
#define __PROC__    __PROC_X86_64__
#endif

//#pragma message("Defining Word_t to 64bit")
typedef uint64_t Word_t, *PWord_t;
typedef int64_t SWord_t, *PSWord_t;

#ifndef JU_KM
    //#pragma message ("Detected Judy User Mode")
    #define JudyPrintf(X) printf(X)
    #define JudyExit(X) exit(X)
    #define JudyFPrintf(X) fprintf(X)
#else //JU_KM
    //#pragma message ("Detected Judy Kernel Mode")
#ifdef __LINUX__
    #define assert(cond)            OSL_BUG_ON(!(cond))
#endif
    #define JudyExit(X)
    #define JudyFPrintf(X)
    #ifndef JU_WIN
        #define printf(...) printk(KERN_INFO __VA_ARGS__)
        #define JudyPrintf(...) printk(KERN_INFO __VA_ARGS__)
    #else
    #define JudyPrintf(X) (0)
    #endif //JU_WIN
#endif //JU_KM

#ifdef JU_64BIT
    //#pragma message ("Detected Judy 64bit")
#else
    //#pragma message ("Detected Judy 32bit")
#endif
...
#ifdef JU_KM
#define JUDYERROR(CallerFile, CallerLine, JudyFunc, JudyErrno, JudyErrID) OSL_BUG_ON(1)
#else //JU_KM
#define JUDYERROR(CallerFile, CallerLine, JudyFunc, JudyErrno, JudyErrID) \
	    {                                                                     \
        	(void) JudyFPrintf(stderr, "File '%s', line %d: %s(), "           \
           		"JU_ERRNO_* == %d, ID == %d\n",                               \
