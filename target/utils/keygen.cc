	...
	struct keygen_option *opt)
{
	...
	//    printf("%d --- %d\n", (int)nprefix, (int)prefix_dist->type);
}
...
size_t keygen_seed2key(struct keygen *keygen, uint64_t seed, char *buf)
{
	...
	for (i = 0; i < keygen->nprefix; ++i) {
		if (i + 1 == keygen->nprefix) {
			...
			// JP: a workaround for key length overflow check in uniform distribution
			if (len > keygen->prefix_len[i].b)
...
{
	char key[100];
	snprintf(key, sizeof(key), "%016d", (int)idx);
