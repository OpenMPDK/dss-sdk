...
log_file = '/var/log/dragonfly/downgrade.log'
...
etcdproxy_service = "etcd-proxy.service"
services=[agent_service, target_service, etcd_service, rest_service,
          monitor_service, etcdgw_service, etcdproxy_service]
...
def update_progress_in_db(module, status):
    cmd = ("ETCDCTL_API=3 etcdctl put /software/downgrade/progress/node/" +
...
def check_services_after_upgrade():
    ...
    """
    for svc in services:
        if os.access(service_dir + svc, os.R_OK):
            ret = service_cmd(svc, 'status')
            if ret != 0:
                logging.error('Service %s not running', svc)
                return False
    return True


def check_and_downgrade_package(package_names):
    packages_upgraded = []
    if 'etcd' in package_names and 'etcd' in installed_versions:
        ...
        if release_versions['etcd'] < installed_versions['etcd']:
            ...
            if not healthy:
                update_progress_in_db('failure_reason',
                                      'cluster unhealthy in pre-downgrade')
    ...
    for key, value in packages.iteritems():
        if key not in package_names:
            logging.info('Skipping package %s', key)
            continue
        if key in installed_versions:
            update_progress_in_db(value, 'started')
            if release_versions[key] < installed_versions[key]:
                ret, _, _ = uninstall_rpm(package_name_file[key])
                if ret:
                    logging.error('Failed to uninstall RPM %s', key)
            ...
            update_progress_in_db(value, 'downgraded')
        else:
            logging.info('Package %s not installed', key)
...
def check_compatibility_for_rollback():
    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument("package_names", type=str,
                        help="display a list of given names")
    args = parser.parse_args()
    host_package_names = map(str, args.package_names.strip('[]').split(','))

    ret = check_compatibility_for_rollback()
    if not ret:
        logging.error('Not compatible for rollback')
        sys.exit(-1)
    ...
    for f in release_versions.keys():
        ...
        if ver and len(ver):
            ...
            if release_versions[name] > installed_versions[name]:
                logging.error('Version for %s [%s] is greater than the '
    ...
    ret_code = check_and_downgrade_package(host_package_names)
    if ret_code:
